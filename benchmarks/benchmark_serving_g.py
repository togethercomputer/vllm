...
=====================================
Example Google Cloud command
export OPENAI_API_KEY=<your Together API key>
=====================================
python3 ./benchmark_serving_g.py \
    --backend=chat_completions \
    --host=https://api.together.xyz \
    --endpoint=v1/chat/completions \
    --model=gcloud-vertex/meta-llama-3-1-throughput-copy \
...
curl -iX POST "https://api.together.xyz/v1/chat/completions" \
...
def format_stream_payload(request_input: RequestFuncInput) -> dict[str, Any]:
...
async def send_stream_request(
  ...
  async with sem:
    async with aiohttp.ClientSession(timeout=AIOHTTP_TIMEOUT) as session:
      payload = format_stream_payload(request_input)
